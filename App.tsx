...
import { 
  Users, Trophy, Loader2, Plus, Trash2, Edit3, Swords, Shield, CheckCircle, X, Calendar, Clock, ChevronRight
} from 'lucide-react';
          ...
          return (
            <div key={t.id} className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group">
              <div className="flex justify-between items-start mb-6">
                <div className="p-4 rounded-3xl bg-blue-50 text-blue-600 dark:bg-blue-900/20 shrink-0">
                    <Trophy size={32}/>
                </div>
...
const AdminConsole = ({ userProfile }: { userProfile: any }) => {
    ...
    const handleSave = async (e: any) => {
        ...
        try {
            if (selectedT) {
                await supabase.from('tournaments').update(data).eq('id', selectedT.id);
            } else {
                await supabase.from('tournaments').insert({ ...data, organizer_id: userProfile.id });
            }
            setFormOpen(false);
            fetchData();
    ...
    if (activeGame) {
        return (
            <ChessBoard 
                ...
                playerBottom={{ name: userProfile?.username || 'Vous', elo: userProfile?.elo_rapid || 1200 }}
                ...
                <div className="lg:col-span-8 bg-white dark:bg-slate-900 p-10 lg:p-14 rounded-[56px] shadow-sm border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="space-y-12">
                        ...
                        <button onClick={startGame} className="w-full py-8 bg-blue-600 text-white rounded-[40px] shadow-3xl flex flex-col items-center justify-center transform active:scale-95 transition-all group hover:bg-blue-700">
                            ...
                            <span className="text-3xl font-black tracking-tighter uppercase">Démarrer</span>
                        </button>
                    </div>
                </div>
                <div className="lg:col-span-4 space-y-4 h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    {MOCK_BOTS.map(b => (
                        <div 
                            ...
                            className={`p-6 rounded-[32px] cursor-pointer border-2 transition-all ${bot.id === b.id && mode === 'bot' ? 'bg-blue-600 text-white border-blue-600 shadow-xl shadow-blue-500/20' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 hover:border-blue-500/40'}`}
                        >
